F_translate_headers <- function(header, standard = c("MARKER", "EFFECTALLELE", "OTHERALLELE", "BETA", "SE", "Z", "P", "LP", "EAF", "N", "NSTUDIES", "QHET", "QHETP", "I2HET", "DIRECTION"), alternative) {
...
meta.min.1 <- function(meta_cohort, metamethod = "FIV", lambda.meta = 1, lambda.cohort = 1, gc_meta = TRUE, calculate_lambda.meta = TRUE, calculate_lambdas.cohort = TRUE) {
  ...
  print(paste(" - Pre-set genomic control lambda of the meta-results = ",format(lambda.meta, digits=3), sep=""), quote=FALSE)
  if (calculate_lambdas.cohort) {
    lambda.cohort <- GClambda(meta_cohort$P.y)
    print(paste(" - Genomic control lambda calculated from the cohort results = ",format(lambda.cohort, digits=3), sep=""), quote=FALSE)
  } else {
    print(paste(" - Pre-set genomic control lambda of the cohort results = ",format(lambda.cohort, digits=3), sep=""), quote=FALSE)
  }
      ...
      if ("BETA.y" %in% names(meta_cohort) & "SE.y" %in% names(meta_cohort)) {
        ...
        if (gc_meta) {
          if (calculate_lambda.meta) {
            ...
            print(paste(" - Genomic control lambda calculated from the corrected meta-results = ",format(lambda.meta.adj, digits=3), sep=""), quote=FALSE)
          } else {
            lambda.meta.adj <- lambda.meta
            print(paste(" - Pre-set genomic control lambda applied to the corrected meta-results = ",format(lambda.meta.adj, digits=3), sep=""), quote=FALSE)
          }
        ...
        adj <- meta_cohort[, c("BETA.adj", "SE.adj", "P.adj", "LP.adj", "EAF.adj", "N.adj", "NSTUDIES.adj")]
      } else { 
    ...
    if ("Z.x" %in% names(meta_cohort) & "N.x" %in% names(meta_cohort)) {
      if ("Z.y" %in% names(meta_cohort) & "N.y" %in% names(meta_cohort)) {
        ...
        meta_cohort$N.adj[subset] <- meta_cohort$N.adj[subset]-meta_cohort$N.y[subset]
        meta_cohort$NSTUDIES.adj[subset] <- meta_cohort$NSTUDIES.adj[subset]-1
        adj <- meta_cohort[, c("Z.adj", "P.adj", "LP.adj", "EAF.adj", "N.adj", "NSTUDIES.adj")]
      } else { 
  ...
  print(paste(" - Genomic control lambda calculated of the final corrected meta-results = ",format(lambda.meta.adj, digits=3), sep=""), quote=FALSE)
  return(adj)
}
